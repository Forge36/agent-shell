...
*** Managing Multiple Agent Shells

=M-x agent-shell-list= - Display a tabulated list of all active agent shells.

The agent shell list provides a centralized view to manage multiple agent sessions across different projects, similar to =list-packages= or =list-processes=. Features include:

- View all active agents with their model, mode, status, project, directory, and activity
- Auto-refresh while the buffer is visible
- Quick access to shell, viewport, and transcript buffers
- Interrupt or kill agents
- Start new shells in the same project

*Key bindings in agent-shell-list:*

- =RET= - Switch to shell or viewport (respects =agent-shell-prefer-viewport-interaction=)
- =o= - Switch to shell buffer directly
- =O= - Switch to viewport buffer
- =t= - Open transcript
- =N= - Start new shell in same project (with prefix arg, choose different agent type)
- =k= - Kill agent buffer
- =g= - Refresh list
- =q= - Quit window
